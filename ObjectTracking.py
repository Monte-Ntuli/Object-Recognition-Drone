...
def getContours(img, imgContour):
    ...
    for cnt in contours:
        ...
        if area > areaMin:
            ...
            # GET OBJECT POSITION (lEFT RIGHT UP AND DOWN)
            if (cx < int(frameWidth / 2) - deadZone):
                # PRINT DRONE DIRECTION ON SCREEN
        ...
        # GET OBJECT POSITION FORWARD AND BACKWARD POSITION
        if (area <= 2300):
            # PRINT DRONE DIRECTION ON SCREEN
            cv2.putText(imgContour, " GO Forward ", (450, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 3)
            cv2.rectangle(imgContour, (int(frameWidth / 2 - deadZone), int(frameHeight / 2) + deadZone),
                          (int(frameWidth / 2 + deadZone), frameHeight), (0, 0, 255), cv2.FILLED)
            fir = 5
        elif area == 3500:
            ...
            cv2.putText(imgContour, " GO backward ", (450, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 3)
            cv2.rectangle(imgContour, (int(frameWidth / 2 - deadZone), int(frameHeight / 2) + deadZone),
                          (int(frameWidth / 2 + deadZone), frameHeight), (0, 0, 255), cv2.FILLED)
            fir = 8
        else:
...
while True:
    ...
    # GET AND PROCESS THE IMAGE FROM TELLO
    ...
    if fir == 5:
        me.for_back_velocity = 35  # Forward and Backward speed
    elif fir == 6:
        me.for_back_velocity = -10  # stop
    elif fir == 8:
        me.for_back_velocity = -35  # Forward and Backward speed

    if dir == 1:
        me.yaw_velocity = -35  # left rotation speed
    elif dir == 2:
        me.yaw_velocity = 35  # Right and rotation speed
    elif dir == 3:
        me.up_down_velocity = 25  # up and down speed
    elif dir == 4:
        me.up_down_velocity = -25  # up and down speed
    else:
    ...
    # land if there is a problem
    if cv2.waitKey(1) & 0xFF == ord('l'):
        me.land()

    # land if connection is lost
    if me.connect() == False:
        me.land()
        break
